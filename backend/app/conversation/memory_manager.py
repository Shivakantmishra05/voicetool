        ...
        for field, value in updates.items():
            ...
            if field == "conversation_metrics":
                ...
                if merged != memory.get("conversation_metrics"):
                    ...
                    changed = True
                continue
            if field in {"language_locked"} and value != memory.get(field):
            ...
            if field == "language" and value in {"hinglish", "hindi", "english"} and value != memory.get(field):
                ...
                changed = True
                continue
            if field in {"lead_score"} and value != memory.get(field):
...
def _merge_defaults(memory: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(asked, dict):
        merged["asked_questions"] = dict(asked)
    else:
        merged["asked_questions"] = list(asked)
